#!/usr/bin/env python3
"""
experiments/multitask_swinunet/train.py
End-to-End Multi-Task Swin-UNet Training (Segmentation + Classification).

Chạy thử (smoke-test 5 batch):
  ./.venv/bin/python experiments/multitask_swinunet/train.py --smoke-test

Chạy thật:
  CUDA_VISIBLE_DEVICES=0 ./.venv/bin/python experiments/multitask_swinunet/train.py
"""
...
for p in [str(ROOT), str(ROOT / "models" / "swin_unet")]:
...
def train(args: argparse.Namespace) -> None:
    ...
    IMG_SIZE = 224       # Swin-UNet yêu cầu input là bội số của window_size (7*32=224)
    ...
    print(f"\n{'='*70}")
    print(f" 🐛 Multi-Task Swin-UNet — End-to-End Training")
    ...
    out_dir  = ROOT / "runs" / "multitask_swinunet" / run_tag
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        pbar = tqdm(
            enumerate(dl_train),
            total=n_batches,
            desc=f"[Swin-UNet] Epoch {epoch:02d}/{EPOCHS} ({pct_epoch:5.1f}%) [TRAIN]",
            ncols=110,
            leave=True,
        )
        ...
        print(
            f"  [Swin-UNet] Ep {epoch:02d}/{EPOCHS} "
            f"({pct_epoch:5.1f}%) | "
    ...
    if not args.smoke_test:
        print(f"\n✅ [Swin-UNet] Hoàn thành! Best score={best_score:.4f} | Saved: {out_dir}")
    else:
        print(f"\n✅ [Swin-UNet] Smoke-test PASSED!")
...
def main():
    p = argparse.ArgumentParser(description="Multi-Task Swin-UNet Trainer")
