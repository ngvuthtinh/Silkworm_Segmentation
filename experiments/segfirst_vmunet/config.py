"""
config.py — Configuration for Segmentation-First Multi-Task VM-UNet experiment.

All hyperparameters and paths are defined here.
Results are saved to runs/segfirst_vmunet/<timestamp>/ automatically.
"""
...
class SegFirstConfig:
    ...
    Results are automatically saved to runs/segfirst_vmunet/<timestamp>/.
    """
    ...
    freeze_backbone_phase2: bool = False   # If True, freeze backbone in Phase 2
    lr_backbone: float = 1e-5             # Small LR for backbone
    ...
    max_cls_samples: int | None = 500      # Subsample Dataset B (None = use all)
    num_workers: int = 4
    seed: int = 42
    ...
    # Output & GPU
    # work_dir is auto-set to runs/segfirst_vmunet/<timestamp>/ in __post_init__
    ...
    def __post_init__(self):
        if not self.work_dir:
            ...
            self.work_dir = os.path.join(
                "runs", "segfirst_vmunet", timestamp
            )
    ...
    def vis_dir(self) -> str:
        return os.path.join(self.work_dir, "test_outputs")
