"""
dataset.py — Dataset classes for Segmentation-First Multi-Task Swin-Unet.

Dataset A (Segmentation-only):
    Loads (image, mask) pairs from silkworm_mixed_dataset or Silkynet layout.
...
class SilkynetSegDataset(Dataset):
    """
    Dataset A: Segmentation dataset.
    Loads (image, mask) pairs from silkworm_mixed_dataset or silkynet layout.
    """
    ...
    def __init__(self, data_root: str, img_size: int = 224, is_train: bool = True):
        ...
        # 1. Standard train/valid/test layout
        ...
        if split_img_dir.exists() and split_mask_dir.exists():
            for img_p in sorted(split_img_dir.iterdir()):
                if img_p.suffix.lower() not in {".jpg", ".png", ".jpeg", ".bmp", ".webp"}:
                    continue
                stem = img_p.stem
                mask_p = None
            ...
            # 2. Fallback to legacy silkynet layout
            pairs = [
