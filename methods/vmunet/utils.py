...
class BCELoss(nn.Module):
    ...
    def forward(self, pred, target):
        size = pred.size(0)
        pred_ = torch.clamp(pred.view(size, -1), 1e-7, 1.0 - 1e-7)
        target_ = torch.clamp(target.view(size, -1), 0.0, 1.0)
...
class myToTensor:
    ...
    def __call__(self, data):
        ...
        mask_t = torch.tensor(np.array(mask), dtype=torch.float32)
        if mask_t.ndim == 3:
...
class myResize:
    ...
    def __call__(self, data):
        image, mask = data
        image, mask = _ensure_pil(image), _ensure_pil(mask)
        res_img = TF.resize(image, [self.size_h, self.size_w])
        res_msk = TF.resize(mask, [self.size_h, self.size_w], interpolation=TF.InterpolationMode.NEAREST)
...
class myColorJitter:
    ...
    def __call__(self, data):
        ...
        if random.random() < self.p:
            image = TF.adjust_brightness(image, random.uniform(1.0 - self.brightness, 1.0 + self.brightness))
            image = TF.adjust_contrast(image, random.uniform(1.0 - self.contrast, 1.0 + self.contrast))
        return image, mask


class myRandomAffine:
    def __init__(self, p=0.5, degrees=(-20, 20), translate=(0.1, 0.1), scale=(0.85, 1.15)):
        self.p = p
    ...
    def __call__(self, data):
        ...
        if random.random() < self.p:
            ...
            image = TF.affine(image, angle=angle, translate=translations, scale=scale_factor, shear=0)
            mask = TF.affine(mask, angle=angle, translate=translations, scale=scale_factor, shear=0)
        return image, mask 
...
class myNormalize:
    ...
    def __call__(self, data):
        ...
        img_normalized = ((img_normalized - np.min(img_normalized)) 
                            / (np.max(img_normalized) - np.min(img_normalized))) * 255.0
